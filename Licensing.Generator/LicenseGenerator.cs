...
namespace Licensing.Generator
{
    ...
    public class LicenseGenerator
    {
        ...
        /// </summary>
        /// <returns>item1是公钥，item2是私钥</returns>
        public static ValueTuple<string, string> GenerateKeyAsync(int keySize = 4096, CspParameters cspParameters = null)
        {
            var rsa = cspParameters == null ? new RSACryptoServiceProvider(keySize) : new RSACryptoServiceProvider(keySize, cspParameters);
            string publicKey = RSAKeyExtensions.ToXmlString(rsa, false);
            string privateKey = RSAKeyExtensions.ToXmlString(rsa, true);
